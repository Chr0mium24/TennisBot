...
5. Validate that stereo `left`/`right` devices match the `source_device` recorded in the `cam1`/`cam2` mono packages.
6. Update README/docs to state that the mainline GUI now uses the legacy CameraCalibLab auto-capture behavior.
7. Run the calibration tests with `uv` and commit all resulting changes.
