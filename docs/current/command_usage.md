...
构建 ROS interface、adapter 和 headless vision 包：
...
- `tennisbot_interface_adapter` 订阅 `/robot/chassis_position` 和
  `/robot/chassis_state`
- adapter 发布 `/vision/chassis_position`、`/vision/chassis_pose` 和
  `/target/raw`
- `tennisbot_headless_vision` 订阅 `/vision/chassis_pose`
- headless 节点发布 `/vision/target_prediction`
- 名义频率：vision raw/pose 30 Hz，managed target 最高 10 Hz
